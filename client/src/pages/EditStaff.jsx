...
export default function EditStaff() {
  ...
  const loadStaffData = async () => {
    try {
      ...
      // Check if role is custom (not in predefined list)
      const predefinedRoles = ['RN', 'RCIS', 'Tech', 'RT', 'EP Tech', 'APP', 'NP'];
      const isCustomRole = !predefinedRoles.includes(staff.role);

      if (isCustomRole) {
        setCustomRole(staff.role);
        setFormData({ ...staff, role: 'Other' });
      } else {
        setFormData(staff);
      }
    } catch (err) {
  ...
  if (loading) {
    return <div className="text-center py-12">Loading staff information...</div>;
  }
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="mb-6">
        ...
        <h2 className="text-3xl font-bold text-gray-900">Edit Staff Member</h2>
      </div>
